...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        ...
        <div class="bg-surface-soft p-5 rounded-3xl border border-neutral-200 shadow-soft flex items-center justify-between">
            <div class="space-y-1">
                <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Invoice</span>
                <h3 class="font-display font-extrabold text-2xl text-slate-900">{{ number_format($stats['total_count']) }}</h3>
                <div class="flex items-center gap-2 text-[10px] font-semibold text-slate-500">
                    ...
                    <span class="text-rose-700 font-bold">{{ $stats['unpaid_count'] }} Belum</span>
                </div>
            </div>
        ...
        <div class="bg-surface-soft p-5 rounded-3xl border border-neutral-200 shadow-soft flex items-center justify-between">
            <div class="space-y-1">
                <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Nilai Tagihan</span>
                <h3 class="font-display font-extrabold text-2xl text-slate-900">Rp {{ number_format($stats['total_amount'], 0, ',', '.') }}</h3>
                <span class="text-[10px] text-slate-500 font-medium">Akumulasi seluruh transaksi</span>
            </div>
        ...
        <div class="bg-surface-soft p-5 rounded-3xl border border-neutral-200 shadow-soft flex items-center justify-between">
            <div class="space-y-1">
                <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Terbayar (Masuk)</span>
                <h3 class="font-display font-extrabold text-2xl text-emerald-700">Rp {{ number_format($stats['total_paid'], 0, ',', '.') }}</h3>
                <span class="text-[10px] text-emerald-800 font-medium">Uang muka (DP) + Pelunasan</span>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center shrink-0 border border-emerald-100">
        ...
        <div class="bg-surface-soft p-5 rounded-3xl border border-neutral-200 shadow-soft flex items-center justify-between">
            <div class="space-y-1">
                ...
                <h3 class="font-display font-extrabold text-2xl text-rose-600">Rp {{ number_format($stats['total_remaining'], 0, ',', '.') }}</h3>
                <span class="text-[10px] text-rose-700 font-medium">Menunggu pelunasan klien</span>
            </div>
            <div class="w-12 h-12 rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center shrink-0 border border-rose-100">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4 bg-surface-soft p-5 rounded-3xl border border-neutral-200 shadow-soft">
        <!-- Search & Filter Form -->
        <form action="{{ route('admin.invoices.index') }}" method="GET" class="flex flex-wrap items-center gap-2.5 flex-1">
            ...
            <select name="status" onchange="this.form.submit()" class="px-3 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white outline-none cursor-pointer">
            ...
            <button type="submit" class="px-4 py-2.5 bg-slate-900 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition">
                Filter
            </button>

            @if(request()->hasAny(['search', 'status', 'date_from', 'date_to']))
                <a href="{{ route('admin.invoices.index') }}" class="px-3 py-2.5 text-xs font-semibold text-slate-500 hover:text-slate-900 transition">
                    Reset
                </a>
            @endif
        </form>
