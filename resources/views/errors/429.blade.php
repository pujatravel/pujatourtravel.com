...
@section('title', '429 - Batas Akses Terlampaui')
@section('description', 'Sistem mendeteksi frekuensi permintaan yang terlalu cepat. Mohon tunggu beberapa saat sebelum mencoba kembali.')
...
<div class="relative bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-10 lg:p-12 shadow-soft border border-neutral-200 overflow-hidden text-center">
    ...
    <div class="relative z-10">
        <!-- Status Pill Badge -->
        <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-purple-50 border border-purple-200/80 text-purple-900 text-xs font-bold uppercase tracking-wider mb-5 sm:mb-6 shadow-2xs">
        ...
        <div class="relative w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 mx-auto mb-5 sm:mb-6 flex items-center justify-center">
            <div class="absolute inset-0 rounded-2xl sm:rounded-3xl bg-purple-500/10 animate-ping opacity-30"></div>
            <div class="w-full h-full rounded-2xl sm:rounded-3xl bg-purple-50 border border-purple-200 text-purple-700 flex items-center justify-center shadow-xs">
                <i data-lucide="gauge" class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 stroke-[1.5]"></i>
            </div>
        </div>

        <!-- Headline & Description -->
        <h1 class="font-display font-extrabold text-xl sm:text-3xl lg:text-4xl text-slate-900 tracking-tight leading-tight">
            Lalu Lintas Sedang Sangat Padat
        </h1>
        <p class="text-slate-600 text-xs sm:text-sm mt-2.5 sm:mt-3 max-w-lg mx-auto leading-relaxed">
            Sistem mendeteksi aktivitas klik atau pengiriman formulir terlalu beruntun dalam waktu singkat. Demi menjaga keadilan kuota server untuk semua wisatawan, mohon beri jeda beberapa saat.
        </p>

        <!-- Action CTA Buttons -->
        <div class="flex flex-col xs:flex-row flex-wrap items-stretch xs:items-center justify-center gap-2.5 sm:gap-3 mt-6 sm:mt-7">
            <button onclick="window.location.reload()" class="w-full xs:w-auto px-5 sm:px-6 py-2.5 sm:py-3 min-h-11 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs sm:text-sm transition-all duration-300 flex items-center justify-center gap-2 shadow-xs hover:-translate-y-0.5 cursor-pointer">
            ...
            <a href="https://wa.me/{{ $waNum ?? '6281234567890' }}?text=Halo%20Admin%20Puja%20Tour,%20saya%20terkena%20limit%20akses%20di%20website" 
               target="_blank" 
        ...
        <div class="mt-8 pt-7 border-t border-neutral-100 text-[11px] text-slate-400">
            <p>Batas kecepatan (rate limiting) akan otomatis dibuka kembali dalam kurun waktu sekitar 30 - 60 detik.</p>
        </div>
    </div>
</div>
@endsection
