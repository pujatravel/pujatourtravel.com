...
<div class="relative bg-white rounded-3xl p-7 sm:p-12 shadow-soft border border-neutral-200 overflow-hidden text-center">
    ...
    <div class="relative z-10">
        <!-- Status Pill Badge -->
        <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-50 border border-emerald-200/80 text-emerald-800 text-xs font-bold uppercase tracking-wider mb-6 shadow-2xs">
        ...
        <div class="relative w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-6 flex items-center justify-center">
            <div class="absolute inset-0 rounded-3xl bg-emerald-500/10 animate-ping opacity-40"></div>
            <div class="w-full h-full rounded-3xl bg-emerald-50 border border-emerald-200 text-emerald-700 flex items-center justify-center shadow-xs">
                <i data-lucide="compass" class="w-10 h-10 sm:w-12 sm:h-12 stroke-[1.5]"></i>
            </div>
        </div>
        ...
        <div class="mt-7 max-w-md mx-auto">
            <form action="{{ route('packages.index') }}" method="GET" class="flex items-center gap-2 bg-slate-50 p-1.5 sm:p-2 rounded-2xl border border-neutral-200 shadow-2xs focus-within:ring-2 focus-within:ring-emerald-700 focus-within:bg-white transition-all">
                <div class="flex-1 flex items-center gap-2 pl-3">
                    ...
                    <input type="text" name="search" placeholder="Cari Green Canyon, Rafting, Snorkeling..." class="w-full text-xs sm:text-sm text-slate-800 placeholder-slate-400 outline-none bg-transparent font-medium">
                </div>
                <button type="submit" class="px-4 sm:px-5 py-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs transition shadow-xs">
                    Cari
                </button>
            </form>
        </div>
