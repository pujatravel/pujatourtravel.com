            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <div>
                    ...
                    <select name="unit_id" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-emerald-600 focus:border-emerald-600 outline-none">
                        @foreach($units as $u)
                            <option value="{{ $u->id }}" {{ old('unit_id', $package->unit_id) == $u->id || (empty(old('unit_id', $package->unit_id)) && strtolower($package->price_unit) == strtolower($u->name)) ? 'selected' : '' }}>/ {{ $u->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
