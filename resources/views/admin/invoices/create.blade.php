    ...
    <form action="{{ route('admin.invoices.store') }}" method="POST" id="invoice-form" class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-surface-soft p-5 rounded-3xl border border-neutral-200 shadow-soft">
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.invoices.index') }}" class="p-2.5 rounded-xl bg-canvas text-slate-500 hover:text-slate-900 border border-neutral-200 transition">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto">
                <a href="{{ route('admin.invoices.index') }}" class="flex-1 sm:flex-none px-4 py-2.5 rounded-xl border border-neutral-200 text-slate-600 font-bold text-xs hover:bg-canvas text-center transition">
                    Batal
                </a>
                ...
                <div class="bg-surface-soft p-6 rounded-3xl border border-neutral-200 shadow-soft space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" name="invoice_number" value="{{ old('invoice_number', $nextInvoiceNumber) }}" required class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-xs font-mono font-bold bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 focus:border-emerald-700 outline-none">
                        ...
                        <div>
                            ...
                            <input type="date" name="invoice_date" value="{{ old('invoice_date', date('Y-m-d')) }}" required class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 focus:border-emerald-700 outline-none">
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date" value="{{ old('due_date', date('Y-m-d', strtotime('+3 days'))) }}" class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 focus:border-emerald-700 outline-none">
                ...
                <div class="bg-surface-soft p-6 rounded-3xl border border-neutral-200 shadow-soft space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="customer_name" value="{{ old('customer_name') }}" placeholder="Contoh: Bpk. Hendra Gunawan / PT Sumber Berkah" required class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 focus:border-emerald-700 outline-none">
                        ...
                        <div>
                            ...
                            <input type="text" name="customer_phone" value="{{ old('customer_phone') }}" placeholder="Contoh: 081234567890" required class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 focus:border-emerald-700 outline-none">
                        ...
                        <div>
                            ...
                            <input type="email" name="customer_email" value="{{ old('customer_email') }}" placeholder="Contoh: customer@email.com" class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 focus:border-emerald-700 outline-none">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Kota Asal / Alamat</label>
                            <input type="text" name="customer_address" value="{{ old('customer_address') }}" placeholder="Contoh: Bandung / Jakarta Selatan" class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 focus:border-emerald-700 outline-none">
                        </div>
                    </div>

                    <!-- Quick Package Selector & Booking Details -->
                    <div class="p-4 rounded-2xl bg-canvas border border-neutral-200 space-y-4 mt-2">
                        <div class="flex items-center justify-between">
                            <label class="block text-xs font-bold text-emerald-800 uppercase flex items-center gap-1.5">
                                <i data-lucide="palmtree" class="w-4 h-4"></i>
                                <span>Pilih Cepat dari Paket Wisata Aktif:</span>
                            </label>
                            <span class="text-[11px] text-slate-400">Opsional (Mengisi otomatis rincian)</span>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div class="sm:col-span-2">
                                <select id="package-selector" name="package_id" class="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-xs bg-white focus:ring-2 focus:ring-emerald-700 outline-none">
                                    <option value="">-- Pilih Paket atau Buat Kustom Sendiri --</option>
                                    @foreach($packages as $pkg)
                                        <option value="{{ $pkg->id }}" 
                                                ...
                                                {{ (old('package_id') == $pkg->id || (isset($selectedPackage) && $selectedPackage->id == $pkg->id)) ? 'selected' : '' }}>
                            ...
                            <div>
                                <input type="text" id="package-name-input" name="package_name" value="{{ old('package_name', $selectedPackage->name ?? '') }}" placeholder="Nama Paket / Trip" class="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-xs bg-white focus:ring-2 focus:ring-emerald-700 outline-none">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="travel_date" value="{{ old('travel_date', date('Y-m-d', strtotime('+7 days'))) }}" class="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-xs bg-white focus:ring-2 focus:ring-emerald-700 outline-none">
                            </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <input type="number" id="pax-count-input" name="pax_count" value="{{ old('pax_count', 1) }}" min="1" required class="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-xs font-bold bg-white focus:ring-2 focus:ring-emerald-700 outline-none">
                ...
                <div class="bg-surface-soft p-6 rounded-3xl border border-neutral-200 shadow-soft space-y-4">
                    <div class="flex items-center justify-between border-b border-neutral-200 pb-3">
                        <div class="space-y-0.5">
                            ...
                            <p class="text-xs text-slate-400">Tambahkan paket, akomodasi, transportasi, makanan, atau tiket</p>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs min-w-[600px]" id="items-table">
                            ...
                            <tbody class="divide-y divide-neutral-200 text-slate-700" id="items-tbody">
                                ...
                                <tr class="item-row">
                                    <td class="p-3">
                                        <input type="text" name="items[0][item_name]" value="{{ old('items.0.item_name', 'Paket Wisata Pangandaran') }}" placeholder="Nama layanan / paket" required class="item-name w-full px-3 py-2 rounded-lg border border-neutral-200 text-xs font-semibold bg-canvas focus:bg-white outline-none">
                                        <input type="text" name="items[0][description]" value="{{ old('items.0.description', '') }}" placeholder="Keterangan tambahan (opsional)" class="w-full px-3 py-1.5 rounded-lg border border-neutral-200 text-[11px] bg-canvas focus:bg-white outline-none mt-1 text-slate-500">
                                    </td>
                                    <td class="p-3">
                                        <input type="number" step="any" name="items[0][quantity]" value="{{ old('items.0.quantity', 1) }}" min="0.01" required class="item-qty w-full px-3 py-2 rounded-lg border border-neutral-200 text-xs font-bold bg-canvas focus:bg-white outline-none">
                                    </td>
                                    <td class="p-3">
                                        <input type="text" name="items[0][unit]" value="{{ old('items.0.unit', 'pax') }}" placeholder="pax / unit" required class="item-unit w-full px-3 py-2 rounded-lg border border-neutral-200 text-xs bg-canvas focus:bg-white outline-none">
                                    </td>
                                    <td class="p-3">
                                        <input type="number" step="any" name="items[0][price]" value="{{ old('items.0.price', 0) }}" min="0" required class="item-price w-full px-3 py-2 rounded-lg border border-neutral-200 text-xs font-bold bg-canvas focus:bg-white outline-none">
                                    </td>
                ...
                <div class="bg-surface-soft p-6 rounded-3xl border border-neutral-200 shadow-soft space-y-4">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="notes" rows="3" class="w-full px-4 py-3 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 outline-none leading-relaxed">{{ old('notes', $defaultNotes) }}</textarea>
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Catatan Internal Admin (Rahasia, Tidak Tercetak)</label>
                            <input type="text" name="admin_notes" value="{{ old('admin_notes') }}" placeholder="Contoh: Booking via Kak Rian, guide Kang Asep sudah dihubungi." class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 outline-none">
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-3.5 text-xs">
                        ...
                        <div class="space-y-1">
                            <div class="flex items-center justify-between text-slate-600">
                                ...
                                <input type="number" step="any" id="discount-input" name="discount" value="{{ old('discount', 0) }}" min="0" class="w-32 px-3 py-1.5 text-right font-bold text-rose-600 rounded-lg border border-neutral-200 bg-canvas focus:bg-white outline-none">
                            </div>
                        </div>
                        ...
                        <div class="space-y-1">
                            <div class="flex items-center justify-between text-slate-600">
                                ...
                                <div class="flex items-center gap-1">
                                    <input type="number" step="any" id="tax-input" name="tax_percent" value="{{ old('tax_percent', 0) }}" min="0" max="100" class="w-20 px-2 py-1.5 text-right font-bold rounded-lg border border-neutral-200 bg-canvas focus:bg-white outline-none">
                        ...
                        <div class="flex items-center justify-between p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200">
                            ...
                            <span id="display-grand-total" class="font-display font-extrabold text-lg text-emerald-800">Rp 0</span>
                        </div>

                        <!-- Uang Muka / Terbayar -->
                        <div class="space-y-1.5 pt-2">
                            <label class="block text-xs font-bold text-slate-700 uppercase">Jumlah Terbayar / DP (Rp)</label>
                            <input type="number" step="any" id="paid-input" name="paid_amount" value="{{ old('paid_amount', 0) }}" min="0" class="w-full px-4 py-2.5 rounded-xl border border-neutral-200 text-sm font-bold text-emerald-700 bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 outline-none">
                        ...
                        <div class="flex items-center justify-between p-3.5 rounded-2xl bg-canvas border border-neutral-200">
                            ...
                            <span id="display-remaining" class="font-display font-extrabold text-base text-rose-600">Rp 0</span>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t border-neutral-200 space-y-4 text-xs">
                        <div>
                            ...
                            <select id="status-select" name="status" required class="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-xs font-bold bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 outline-none">
                                ...
                                <option value="PARTIAL" {{ old('status') === 'PARTIAL' ? 'selected' : '' }}>🟡 Uang Muka / DP (Partial)</option>
                        ...
                        <div>
                            ...
                            <input type="text" name="payment_method" value="{{ old('payment_method', 'Transfer Bank (BCA / Mandiri / BRI / QRIS)') }}" placeholder="Contoh: Transfer Bank BCA / QRIS" class="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-xs bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 outline-none">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Rincian Rekening Bank (Tercetak di Invoice)</label>
                            <textarea name="bank_details" rows="4" class="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-[11px] font-mono bg-canvas focus:bg-white focus:ring-2 focus:ring-emerald-700 outline-none leading-relaxed">{{ old('bank_details', $defaultBankDetails) }}</textarea>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateTotals() {
        ...
        // Auto update status dropdown suggestion if user didn't explicitly set cancelled
    ...
    // Event delegation for table inputs & remove button
    ...
    // Package selection auto populate
    if (packageSelector) {
        packageSelector.addEventListener('change', function() {
            ...
            if (firstRow) {
                ...
                firstRow.querySelector('.item-qty').value = pax;
            }
            calculateTotals();
        });
    }

    // Pax input update first row
    if (paxCountInput) {
        paxCountInput.addEventListener('input', function() {
            const val = parseInt(this.value) || 1;
            const firstRow = tbody.querySelector('.item-row');
            if (firstRow && firstRow.querySelector('.item-unit').value.toLowerCase().includes('pax')) {
                firstRow.querySelector('.item-qty').value = val;
                calculateTotals();
            }
        });
    }

    // Recalculate on discount / tax / paid change
