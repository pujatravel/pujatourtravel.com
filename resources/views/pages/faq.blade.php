    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20">
        ...
        <div class="space-y-4">
            @forelse($faqs as $fIndex => $faq)
                <div class="faq-item bg-surface-soft rounded-2xl border border-neutral-200 overflow-hidden shadow-xs {{ $fIndex === 0 ? 'is-active' : '' }}">
                    <button type="button" class="faq-toggle w-full px-6 py-5 text-left flex items-center justify-between gap-4 font-display font-bold text-slate-900 text-base hover:text-emerald-700 transition cursor-pointer">
                        ...
                        <div class="faq-icon w-8 h-8 rounded-full bg-emerald-50 text-emerald-700 flex items-center justify-center shrink-0 transition-transform duration-300 {{ $fIndex === 0 ? 'rotate-180' : '' }}">
