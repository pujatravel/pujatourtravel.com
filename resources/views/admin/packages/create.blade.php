        ...
        <form action="{{ route('admin.packages.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Body Rafting Green Canyon Full Track" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-ocean-500 outline-none">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price') }}" required placeholder="225000" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-ocean-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Durasi Trip</label>
                    <input type="text" name="duration" value="{{ old('duration') }}" placeholder="Contoh: 4 - 5 Jam / 2D1N" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-ocean-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Lokasi Destinasi</label>
                    <input type="text" name="location" value="{{ old('location') }}" placeholder="Contoh: Green Canyon Pangandaran" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-ocean-500 outline-none">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Deskripsi Singkat (Preview Kartu)</label>
                <textarea name="short_description" rows="2" placeholder="Tuliskan 2-3 kalimat ringkas untuk kartu paket..." class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-ocean-500 outline-none">{{ old('short_description') }}</textarea>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Deskripsi Lengkap</label>
                <textarea name="description" rows="4" placeholder="Penjelasan lengkap mengenai aktivitas, pengalaman, dan keistimewaan tur..." class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-ocean-500 outline-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <textarea name="inclusions_text" rows="4" placeholder="Pemandu Lisensi HPI&#10;Helm & Rompi Pelampung&#10;Makan Siang Prasmanan&#10;Asuransi" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:bg-white focus:ring-2 focus:ring-ocean-500 outline-none">{{ old('inclusions_text') }}</textarea>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase">Tidak Termasuk (Exclusions)</label>
                    <p class="text-[11px] text-slate-400 mb-1">Tuliskan 1 item per baris:</p>
                    <textarea name="exclusions_text" rows="4" placeholder="Transportasi dari kota asal&#10;Pengeluaran pribadi" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:bg-white focus:ring-2 focus:ring-ocean-500 outline-none">{{ old('exclusions_text') }}</textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div class="flex items-center pt-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="featured" value="1" {{ old('featured') ? 'checked' : '' }} class="rounded text-ocean-600 focus:ring-ocean-500 w-4 h-4">
